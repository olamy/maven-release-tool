...
public class MavenPrepareStep extends AbstractStep {

    public MavenPrepareStep(CommandRunner runner) {
        super(runner);
    }

    @Override
    public String name() {
        return "maven-release-prepare";
    }

    @Override
    public String describe() {
        return "Run mvn release:prepare (version bumps, SCM tag, clean verify)";
    }
    ...
    public List<String> defaultCommands(ReleaseState state) {
        ...
        if (state.getReleaseTag() != null) {
            parts.add("-Dtag=" + state.getReleaseTag());
        }
